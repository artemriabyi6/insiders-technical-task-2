...
export interface TodoList {
  ...
  title: string;
  ownerId: string;
  collaborators: Record<string, 'admin' | 'viewer'>;
  createdAt?: any;
  updatedAt?: any;
}
...
export function useTodoLists(userEmail: string | null) {
  ...
  const createList = async (title: string, ownerEmail: string) => {
    try {
      await addDoc(collection(db, "todoLists"), {
        title,
        ownerId: ownerEmail,
        collaborators: { [ownerEmail]: 'admin' },
        createdAt: serverTimestamp()
      });
    } catch (err) {
      console.error("Помилка створення списку:", err);
  ...
  const addCollaborator = async (listId: string, email: string, role: 'admin' | 'viewer') => {
    try {
      ...
      console.error("Помилка додавання співавтора:", err);
      throw err;
    }
  };
  ...
  const removeCollaborator = async (listId: string, email: string) => {
    try {
      const ref = doc(db, "todoLists", listId);
      await updateDoc(ref, {
        [`collaborators.${email}`]: deleteField()
      });
    } catch (err) {
      console.error("Помилка видалення співавтора:", err);
      throw err;
    }
  };
